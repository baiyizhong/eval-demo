"""Reference PA remote experiment runner using the Langfuse Python SDK.

This file is intentionally self-contained so an integration team can copy it out
of the repo and replace `run_application` with their own application logic.

Run locally:

    uv run --with fastapi --with uvicorn --with httpx --with langfuse \
      python examples/pa_remote_experiment_runner.py
...
Required environment variables:
    ...
    PA_RUNNER_BEARER_TOKEN=<same token configured in PA scene credentialRef>

Optional environment variables:

    PA_RUNNER_HOST=0.0.0.0
    PA_RUNNER_PORT=8099
    PA_RUNNER_CALLBACK_TIMEOUT_SECONDS=20
    PA_RUNNER_LANGFUSE_VISIBILITY_TIMEOUT_SECONDS=60

PA scene remote trigger URL example:

    http://127.0.0.1:8099/pa/remote-experiment-runs
"""
...
app = FastAPI(title="PA Remote Experiment Runner Reference")
...
@app.post("/pa/remote-experiment-runs")
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "pa_remote_experiment_runner:app",
        host=os.getenv("PA_RUNNER_HOST", "0.0.0.0"),
        port=int(os.getenv("PA_RUNNER_PORT", "8099")),
        reload=False,
    )
