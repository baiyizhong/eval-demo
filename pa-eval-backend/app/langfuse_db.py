...
class LangfuseDatabaseReader:
    def __init__(
        self,
        settings: Settings,
        project_api_client: LangfuseProjectApiClient | None = None,
    ) -> None:
        self._settings = settings
        self._database_url = settings.langfuse_database_url
        self._langfuse_salt = settings.langfuse_salt
        self._project_api_client = project_api_client or LangfuseProjectApiClient(
            settings
        )
    ...
    def _to_pa_evaluator_payload(row: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "provider": row["provider"],
            "evaluationScenario": _evaluation_scenario_from_config(row.get("config")),
            "projectId": row.get("project_id"),
...
def _evaluation_scenario_from_config(value: Any) -> str:
    config = value if isinstance(value, dict) else {}
    scenario = str(config.get("evaluationScenario") or "").strip()
    return scenario or "SINGLE_TURN"
