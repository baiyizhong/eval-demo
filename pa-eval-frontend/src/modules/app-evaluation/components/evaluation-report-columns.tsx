...
export function createEvaluationReportColumns({
  ...
  return [
    {
      accessorKey: 'title',
      header: ({ column }) => <DataTableColumnHeader column={column} title='报告标题' />,
      cell: ({ row }) =>
        row.original.status === 'READY' ? (
          ...
          <LongText className='max-w-72 font-medium'>{row.original.title}</LongText>
        ),
      enableHiding: false,
    },
    {
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title='来源任务' />,
      cell: ({ row }) => <LongText className='max-w-64'>{row.original.sourceTaskName}</LongText>,
    },
    {
      accessorKey: 'status',
      header: ({ column }) => <DataTableColumnHeader column={column} title='状态' />,
      cell: ({ row }) => <EvaluationReportStatusBadge status={row.original.status} />,
    },
    ...
    { accessorKey: 'flowbackCount', header: '已回流' },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title='生成时间' />,
